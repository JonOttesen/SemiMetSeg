...
class DatasetContainer(object):
    ...
    def k_fold(self, folds: int = 5, fold: int = 1, seed: int = 42):
        ...
        splits = list(range(fold*split, (fold + 1)*split))

        # This could be done with a list comprehension, but it is more readable this way
        for i, entry in enumerate(dataset):
            if i in splits:
    ...
    def folder_directory(
        ...
        for folder in subdirs:
            file = glob.glob(str(Path(path) / Path(folder)) + "/*seq.npy")[0]
    ...
    def ELITE(
        ...
        check_contrast = ["ce", "gd", "_c.", "_c_", "post", "t1c"]
