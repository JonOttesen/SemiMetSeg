...
class VolumeInputDropout(monai.transforms.MapTransform):
    ...
    ***monai.transforms.Transform compatible***
    ...
    Input-level dropout
    """
    def __init__(self, keys: KeysCollection, prob: float, num_sequences: int, always_keep: Optional[Union[List[int], int]] = None):
        """
        Args:
            labels tuple(int): The total number of sequences, not the number of classes
        """
        ...
        self.prob = prob
        self.num_sequences = num_sequences
        self.always_keep = [always_keep] if isinstance(always_keep, int) else always_keep
    ...
    def __call__(self, data: Dict[Any, np.ndarray]):
        ...
        # Guarantee that some specified sequences are kept, e.g., post contrast sequences
        if self.always_keep is not None:
            if np.sum([drop[i] for i in self.always_keep]) == len(self.always_keep):
                drop[np.random.choice(self.always_keep)] = False
